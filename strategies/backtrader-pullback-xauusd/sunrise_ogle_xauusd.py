...
if __name__ == '__main__':
    ...
    # ---------------------------------------------------------------
    # CLI overrides (added during port so the strategy runs anywhere
    # without editing module-level constants). Every flag falls back
    # to the constant defined at the top of this file.
    # ---------------------------------------------------------------
    _ap = argparse.ArgumentParser(
        description='Backtest the Sunrise Ogle XAUUSD pullback-window strategy.')
    ...
    _ap.add_argument('--direction', choices=['long', 'short', 'both'], default=None,
                     help='Override trading direction (default: file config)')
    _ap.add_argument('--limit-bars', type=int, default=LIMIT_BARS,
                     help='Stop after N bars (0 = no limit)')
    _ap.add_argument('--quick-test', action='store_true', default=QUICK_TEST,
                     help='Reduce to last 10 days for a fast smoke test')
    _ap.add_argument('--plot', action='store_true', default=ENABLE_PLOT,
    ...
    ENABLE_PLOT = _args.plot
    if _args.quiet:
        VERBOSE_DEBUG = False
    _direction_long_override = None
    _direction_short_override = None
    if _args.direction is not None:
        _direction_long_override = _args.direction in ('long', 'both')
        _direction_short_override = _args.direction in ('short', 'both')
        ENABLE_LONG_TRADES = _direction_long_override
        ENABLE_SHORT_TRADES = _direction_short_override
    ...
    if _args.data:
        ...
        DATA_FILE = SCRIPT_DIR / 'data' / DATA_FILENAME
        if not DATA_FILE.exists():
            # Fall back to the original src/strategy/../../data layout.
            DATA_FILE = Path(__file__).resolve().parent.parent.parent / 'data' / DATA_FILENAME
    STRAT_KWARGS = dict(
    ...
    # Wire CLI direction override into the strategy params (runtime, not class-time).
